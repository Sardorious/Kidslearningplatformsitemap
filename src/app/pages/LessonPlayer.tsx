...
export function LessonPlayer() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Try to find index from URL lessonId, else default to 0
  ...
  if (loading) {
    return <div className="text-center py-20 text-xl font-bold">Loading lessson...</div>;
  }
  ...
  const handleNext = async () => {
    // Attempt to mark current lesson as completed
    try {
      if (!currentLesson.completed) {
        setCompleting(true);
        await userService.completeLesson({ lessonId: currentLesson.id });
        // Optimistically update local state just to show it
