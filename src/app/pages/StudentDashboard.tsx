...
import { studentProgress, lessons } from "../data/mockData";
import { Card } from "../components/ui/card";
import { Button } from "../components/ui/button";
import { Progress } from "../components/ui/progress";
import { useLanguage } from "../contexts/LanguageContext";
import { userService, courseService, lessonService } from "../api/services"; // Added API services
...
export function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Let's assume the user is "enrolled" in the first 2 courses for demo purposes since we lack an enrollment API
        setCourses(coursesData ? coursesData.slice(0, 2) : []);
      } catch (error) {
  ...
  if (loading) {
    return <div className="text-center py-20 text-xl font-bold">Loading dashboard...</div>;
  }

  const completedLessonsCount = progress.filter(p => p.isCompleted).length;
  // Use profile values where available, fallback to mock data for visual completeness
      ...
      <Card className="p-6 bg-gradient-to-r from-purple-500 via-pink-500 to-orange-400 text-white">
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-lg opacity-90 mb-3">{displayProfile.grade} • {t.learningChampion} 🏆</p>
            <div className="flex flex-wrap gap-3 justify-center md:justify-start">
              <div className="bg-white/20 backdrop-blur-sm rounded-xl px-4 py-2">
                <div className="flex items-center gap-2">
                  <Flame className="w-5 h-5" />
                  <span className="font-bold">{displayProfile.streak} {t.dayStreak}</span>
                </div>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-xl px-4 py-2">
                <div className="flex items-center gap-2">
                  <Star className="w-5 h-5" />
                  <span className="font-bold">{displayProfile.points} {t.points}</span>
                </div>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-xl px-4 py-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold">
                    {displayProfile.achievements.filter(a => a.earned).length} Badges
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-5 bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-200">
          ...
          <div className="text-3xl font-black text-green-900 mb-1">
            {displayProfile.completedLessons}
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            ...
            <Card className="divide-y">
              {displayProfile.recentActivity.map((activity, index) => (
            ...
            <Card className="p-4">
              <div className="grid grid-cols-3 gap-3">
                {displayProfile.achievements.map((achievement) => (
                  <div
                    key={achievement.id}
                    className={`aspect-square rounded-xl flex flex-col items-center justify-center p-3 transition-all ${achievement.earned
